...
public class TileEntityHopper extends TileEntity implements Hopper
{
    ...
    private boolean isHopperEmpty(){
    	// Simple check to see if the hopper is empty
    	for (ItemStack is : this.hopperItemStacks)
    		if (is != null)
    			return false;
    	return true;
    }
    ...
    private boolean isHopperFull(){
    	// Simple check to see if the hopper is full
    	for (ItemStack is : this.hopperItemStacks)
    ...
    public boolean updateHopper()
    {
        if (this.worldObj != null && !this.worldObj.isRemote){
            if (!this.isCoolingDown() && BlockHopper.getIsBlockNotPoweredFromMetadata(this.getBlockMetadata())){
            	...
            	if (!this.isHopperEmpty())	// We can't insert if the hopper is empty
            		gotTransaction = this.insertItemToInventory();            	
            	
            	if (!this.isHopperFull())	// We can't pull if the hopper is empty
            		gotTransaction = suckItemsIntoHopper(this) || gotTransaction;            	
    ...
    private static boolean isInventoryFull(IInventory inventory, int side){
    	if (inventory instanceof ISidedInventory  && side > -1){
    	    ISidedInventory sinv = (ISidedInventory)inventory;
    	    int[] islots = sinv.getAccessibleSlotsFromSide(side);
    ...
    private static boolean isInventoryEmpty(IInventory inventory, int side){
    	if (inventory instanceof ISidedInventory  && side > -1){
    	    ISidedInventory sinv = (ISidedInventory)inventory;
    	    int[] islots = sinv.getAccessibleSlotsFromSide(side);
    ...
    private boolean insertItemToInventory()
    {
        ...
        int outputSide        = Facing.oppositeSide[BlockHopper.getDirectionFromMetadata(this.getBlockMetadata())];
        
        if (isInventoryFull(iinventory, outputSide)) return false;	// If the target inventory is full, there is no way to push. Abort.
        
        for (int i = 0; i < this.getSizeInventory(); ++i){
            if (this.getStackInSlot(i) != null){
                ItemStack itemstack = this.getStackInSlot(i).copy();
                ItemStack itemstack1 = insertStack(iinventory, this.decrStackSize(i, 1), outputSide);
    ...
    public static boolean suckItemsIntoHopper(Hopper par0Hopper)
    {
        ...
        if (iinventory != null && isInventoryEmpty(iinventory, 0))	//If the input inventory is empty, nothing to pull, Abort.
        	return false;
